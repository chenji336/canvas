...
canvas.onmouseup=function(e){
	...
	var t1= setInterval(function(){
		...
		for(var i=0;i<track.length;i++){
			 ...
			 context.beginPath();
			 context.moveTo(track[i].lastX,track[i].lastY);
			 context.lineTo(track[i].curX,track[i].curY);

			 var lineWidth=track[i].width;
			 	if(track[p].lastY<track[i].lastY)
			 		p=i;
			...
			g=g+1.5;
			if(p!=-1&&track[p].lastY>=canvasheight){
				context.clearRect(0,0,canvaswidth,canvasheight);
				drawGrid();
				var h=Math.floor(track[p].lastY-canvasheight);
				for(var i=0;i<track.length;i++){
					context.beginPath();
					context.moveTo(track[i].lastX,track[i].lastY-h);
					context.lineTo(track[i].curX,track[i].curY-h);
					var lineWidth=track[i].width;
					context.strokeStyle=strokeColor;
					context.lineWidth=lineWidth;
					context.lineCap="round";
...
canvas.onmousedown=function(e){
	...
	lastLoc=canvasToWindow(e.clientX,e.clientY);
}
